...
t_sector *find_sector(Byte *requested_CHRN)
{
   ...
   #ifdef DEBUG_FDC
   {
	   idx = (((int)requested_CHRN[0]*18)+((int)requested_CHRN[1]*9)+(int)requested_CHRN[2]-1)*512;
   if(requested_CHRN[0]==8)
	   idx+=0;

   fprintf(pfoDebug, "\nFDC Find Sector [0x%02x][0x%02x][0x%02x] A[0x%06x]\n", requested_CHRN[0], requested_CHRN[1], requested_CHRN[2], idx);
   }
	#endif
   return sector;
}
