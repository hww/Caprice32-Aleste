...
union TCounter {
   ...
   uLong Re;
} Ton_Counter_A, Ton_Counter_B, Ton_Counter_C, Noise_Counter;
...
inline void Synthesizer_Mixer_Q(void)
{
   ...
   aleste = SynthesizerMS();
   Left_Chan += (LevL + aleste)/2;
   Right_Chan += (LevR + aleste)/2;
}
...
void Synthesizer_Stereo16(void)
{
   ...
   *(uLong *)CPC.snd_bufferptr = val.d; // write to mixing buffer
   CPC.snd_bufferptr += 4;
   Left_Chan = 0;
