...
#define CYCLE_ADJUST(p) (((uLong)(p) * CYCLE_SCALE) >> 16)
#define MS_TO_CYCLES(p) ((uLong)(p) * 4000)
//#define MS_TO_CYCLES(p) ((uLong)(p) * 3994)
...
word wCycleTable[2];
int iTapeCycleCount;
uLong dwTapePulseCycles;
uLong dwTapeZeroPulseCycles;
uLong dwTapeOnePulseCycles;
uLong dwTapeStage = TAPE_END;
uLong dwTapePulseCount;
uLong dwTapeDataCount;
uLong dwTapeBitsToShift;
   ...
   while (pbTapeBlock < pbTapeImageEnd) { // loop until a valid block is found
      ...
      switch (*pbTapeBlock) {
         ...
         case 0x14: // pure data block
            ...
            dwTapeDataCount = ((*(uLong *)(pbTapeBlock+0x01+0x07) & 0x00ffffff) - 1) << 3; // (Byte count - 1) * 8 bits
         ...
         case 0x15: // direct recording
            ...
            dwTapeDataCount = ((*(uLong *)(pbTapeBlock+0x01+0x05) & 0x00ffffff) - 1) << 3; // (Byte count - 1) * 8 bits
            dwTapeDataCount += *(pbTapeBlock+0x01+0x04); // add the number of bits in the last data Byte
         ...
         case 0x35: // custom info block
            pbTapeBlock += *(uLong *)(pbTapeBlock+0x01+0x10) + 0x14 + 1; // nothing to do, skip the block
            break;

         case 0x40: // snapshot block
            pbTapeBlock += (*(uLong *)(pbTapeBlock+0x01+0x01) & 0x00ffffff) + 0x04 + 1; // nothing to do, skip the block
            break;
         ...
         default: // "extension rule"
            pbTapeBlock += *(uLong *)(pbTapeBlock+0x01) + 4 + 1; // nothing to do, skip the block
      }
   }
...
void Tape_BlockDone(void)
{
   if (pbTapeBlock < pbTapeImageEnd) {
      switch (*pbTapeBlock) {
         ...
         case 0x11: // turbo loading data block
            pbTapeBlock += (*(uLong *)(pbTapeBlock+0x01+0x0f) & 0x00ffffff) + 0x12 + 1;
            break;
         ...
         case 0x14: // pure data block
            pbTapeBlock += (*(uLong *)(pbTapeBlock+0x01+0x07) & 0x00ffffff) + 0x0a + 1;
            break;

         case 0x15: // direct recording
            pbTapeBlock += (*(uLong *)(pbTapeBlock+0x01+0x05) & 0x00ffffff) + 0x08 + 1;
            break;
         ...
         if (dwTapePulseCount > 0) {
            ...
            switch (*pbTapeBlock) {
               ...
               case 0x11: // turbo loading data block
                  ...
                  dwTapeDataCount = ((*(uLong *)(pbTapeBlock+0x01+0x0f) & 0x00ffffff) - 1) << 3; // (Byte count - 1) * 8 bits;
