...
// ----------------------------------------------------------------------------
...

      {
		 ...
		 if(ALESTE & alesteExtend.bit.HIGHTX){
	         addr.b.h &= 0xcF; // WE SHOULD REPLACE V13 BY V11
	         addr.b.h |= (raster_count & 0x30) | (raster_count & 0x08)<<3; // insert rc, masked with 00111000
		 } else {
         ...
         if (VDU.hcount) {
            if ((CRTC.flags & (HDT_flag | VDT_flag)) == (HDT_flag | VDT_flag)) { // DISPTMG enabled?
               if (CRTC.skew) {
			      ...
			      // render one chars line I guess 2 bytes
				   if(ALESTE && alesteExtend.bit.MAPMOD && !alesteExtend.bit.BLACKS)
					  GateArray.scr_border();	
				   else
...
#include "aleste_draw_8bpp.c"

#include "aleste_draw_16bpp.c"

#include "aleste_draw_24bpp.c"

#include "aleste_draw_32bpp.c"
